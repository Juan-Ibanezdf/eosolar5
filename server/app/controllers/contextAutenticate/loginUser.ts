...
export async function loginUser(req: Request, res: Response): Promise<Response> {
  ...
  try {
    ...
    res.cookie('token', token, {
     ...
     // res.cookie('token', token, {
    //   httpOnly: true,
    //   secure: process.env.NODE_ENV === 'production', // Corrigido para ser true em produção
    //   sameSite: 'None', // Corrigido para 'None'
    //   maxAge: 4 * 60 * 60 * 1000, // Quatro horas em milissegundos
    //   path: '/'
    // });
